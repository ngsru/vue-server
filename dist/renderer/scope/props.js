...
module.exports = {
    ...
    buildComputedProps: function buildComputedProps(vm) {
    ...
    pullPropsData: function pullPropsData(vm) {
        var self = this;
    ...
    pullPropsDataItem: function pullPropsDataItem(vm, name, config) {
        ...
        if (descriptor) {
            if (!rawValue) {
                ...
                if (descriptor.required) {
                    vm.__states.$logger.warn('Missing required prop: ' + propName, common.onLogMessage(vm));
                    return;
                }
            } else {
                // Data types
                if (descriptor.type) {
                    ...
                    if (hasTypeError) {
                        vm.__states.$logger.warn('Invalid prop: type check failed for "' + propName + '". Expected ' + expectedTypes.map(this.formatType).join(', ') + ', got ' + this.formatValue(value), common.onLogMessage(vm));
                        return;
                    }
                }
                ...
                if (rawValue && descriptor.validator && !descriptor.validator(value)) {
                    vm.__states.$logger.warn('Invalid prop: custom validator check failed for "' + propName + '"', common.onLogMessage(vm));
                    return;
                }
            }
        }
