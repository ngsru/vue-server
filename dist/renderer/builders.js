...
var builders = {
    build: function build(vm, callback) {
        ...
        process.nextTick(function () {
            ...
            vm.__states.componentsNames = Object.keys(builders.getAsset(vm, 'components'));
    ...
    buildElements: function buildElements(vm, elements, customIndex) {
        var element;
        var repeatElements;
        ...
        for (var i = customIndex || 0, l = elements.length; i < l; i++) {
            ...
            if (element.type === 'tag') {
                ...
                if (element.dirs.for) {
                    ...
                    Array.prototype.splice.apply(elements, [i + 1, element.dirs.for.renderedCount || 0].concat(repeatElements));
                ...
                if (element.name === 'partial') {
                    builders.getPartial({
                        ...
                        onDoesExist: function onDoesExist(partial) {
                            element.inner = partial();
                        },
                        onDoesNotExist: function onDoesNotExist() {
                            element.inner = [];
                        }
                    });
                }
    ...
    getPartial: function getPartial(meta) {
        var vm = meta.vm;
        ...
        if (partial) {
            ...
            if (typeof partial !== 'function') {
                partial = partialAssests[partialName] = asset.compileTemplate(vm.__states.$logger, partial, 'Partial "' + partialName + '"');
            }
    ...
    getForData: function getForData(vm, dir) {
        ...
        if (!value) {
            return value;
        } else if ((type === 'number' || type === 'string') && isNumber.test(value)) {
            array = [];
    ...
    buildComponent: function buildComponent(vm, element, options) {
    ...
    getComponent: function getComponent(vm, component, componentName) {
        var composed;
        if (typeof component === 'function') {
            ...
            composed = asset.composeComponent(vm.__states.$logger, component, vm.$root.__states.mixin);
            builders.getAsset(vm, 'components')[componentName] = composed;
            return composed;
        }
    },

    buildComponentContent: function buildComponentContent(vm, element, options, componentName) {
    ...
    logComponentResolveError: function logComponentResolveError(vm, element, componentName, reason) {
    ...
    buildForElements: function buildForElements(vm, elements, element) {
    ...
    getForItemData: function getForItemData(data, index, directive, key) {
        var forDataItem = {};
    ...
    getAsset: function getAsset(vm, asset) {
