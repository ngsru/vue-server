...
module.exports = {
    getContent: function getContent(vm) {
        var content = null;
    ...
    insert: function insert(vm, slot, content) {
        var slotName = null;
        ...
        for (var j = 0; j < content.length; j++) {
            (function () {
                ...
                if (slotName === elementSlotName && !(element.type === 'text' && String(element.text).trim() === '')) {
                    this.fillSlot(slot, element);
                }
            }).call(this);
        }
    },

    fillSlot: function fillSlot(slot, item) {
        if (!slot.filled) {
            slot.inner = [];
            slot.filled = true;
        }
